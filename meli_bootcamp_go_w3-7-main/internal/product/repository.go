...
type Repository interface {
	...
	GetRecordReportsByProductId(ctx context.Context, productId int) (recordsCount int, err error)
}

const (
	getRecordReportsByProductIdQuery = "SELECT count(pr.id) as product_records_count " +
		"FROM products p " +
		"INNER JOIN product_records pr on p.id = pr.product_id " +
		"WHERE p.id=?"
)
...
func (r *repository) GetRecordReportsByProductId(ctx context.Context, productId int) (int, error) {
	stmt, err := r.db.Prepare(getRecordReportsByProductIdQuery)
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
	...
	var productRecordsCount int
	err = row.Scan(&productRecordsCount)
	if err != nil {
		return 0, err
	}
